...
CSV_COLS = ["Week #","Report #","Report Date","Futures Based On",
...
def get_old_new(cy,cm,ry):
    ...
    return f"new{cy-ry+(1 if cm==12 else 0)}"
...
def parse_report(html):
    ...
    if tables:
        m=re.search(r"as of (\d{1,2}/\d{1,2}/\d{4})",tables[0].get_text())
        if m:
            report_date_str=m.group(1)
            mo,dy,yr=map(int,report_date_str.split("/"))
            release_year=yr
            try: week_num=date(yr,mo,dy).isocalendar()[1]
            except: pass
        rn=re.search(r"Weekly Report\s+(\d+)",html)
        if rn: report_num=int(rn.group(1))
    rows_out=[]
    for table in tables:
        for row in table.find_all("tr"):
            ...
            mon_m=re.search(r"(january|february|march|april|may|june|july|august|september|october|november|december)",label.lower())
            ...
            if is_total:
                rows_out.append({"Week #":week_num,"Report #":report_num,"Report Date":report_date_str,
                    "Futures Based On":"Totals","Unfixed Call Sales":s,"Chg Sales":cs,
                    "Unfixed Call Purchases":p,"Chg Purchases":cp,"At Close":cl,"Chg At Close":cc,
                    "Yr":"","Month":"","Old/New":"total","_ry":release_year})
            elif year_m and mon_m:
                cy=int(year_m.group(1));cm=MONTH_MAP[mon_m.group(1)]
                rows_out.append({"Week #":week_num,"Report #":report_num,"Report Date":report_date_str,
                    "Futures Based On":label.strip(),"Unfixed Call Sales":s,"Chg Sales":cs,
                    "Unfixed Call Purchases":p,"Chg Purchases":cp,"At Close":cl,"Chg At Close":cc,
                    "Yr":cy,"Month":cm,"Old/New":get_old_new(cy,cm,release_year) if release_year else "",
                    "_ry":release_year})
    return rows_out
...
def get_candidate_urls():
    ...
    known_2026=["deaoncall010226.html","deaoncall010826.html","deaoncall011526.html",
...
def append_rows(csv_path,new_rows):
    ...
    for r in new_rows:
        existing_rows.append({k:v for k,v in r.items() if not k.startswith("_")})
    def sk(r):
        try: return datetime.strptime(r.get("Report Date",""),"%m/%d/%Y")
        except: return datetime.min
    existing_rows.sort(key=sk)
...
def main():
    ...
    for url in get_candidate_urls():
        try:
            ...
            ry=rows[0].get("_ry")
            if ry and ry<2016: continue
            rdate=rows[0]["Report Date"]
            if rdate in existing_dates: continue
            new_rows.extend(rows);existing_dates.add(rdate);reports_added+=1
            print(f"✅ NEW: {url.split('/')[-1]} → {rdate} ({len(rows)} rows)")
        except Exception as e:
            print(f"⚠️ {url.split('/')[-1]}: {e}")
        time.sleep(0.3)
    print(f"\nNew reports: {reports_added} | New rows: {len(new_rows)}")
    if new_rows:
